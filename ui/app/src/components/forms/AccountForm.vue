...
const submitForm = async () => {
	...
	const data = {
		...
		openingBalanceDate: openiningBalanceDate.value,
		notes: notes.value
	}
	...
	if (!route.params.id) {
		...
		result = await API.Accounts.edit(data);
	}
...
<template>
	<form class="relative">
		...
		<div
			...
			<Card>
				<template #title>
					<label class="mb-2"> {{ $t('forms.mandatoryFields') }}:</label>
				</template>
				<template #content>
					<hr class="p-3"/>
			...
			<Card>
				...
				<template #content>
					<hr class="p-3"/>
					<TextInput v-model="iban"
					           text="forms.IBAN"
					           placeholder="forms.IBAN"
					           :disabled="loading"/>
					<TextInput v-model="bic"
					           text="forms.BIC"
					           placeholder="forms.BIC"
					           :disabled="loading"/>
					<TextInput v-model="accountNumber"
					           text="forms.accountNumber"
					           placeholder="forms.accountNumberPlaceHolder"
					           :disabled="loading"/>
					<NumberInput v-model="openingBalance"
					             text="forms.openingBalance"
					             placeholder="1000"
					             :minFractionDigits="2"
					             :disabled="loading"/>
					<DateInput v-model="openiningBalanceDate"
					           text="forms.openingBalanceDate"
