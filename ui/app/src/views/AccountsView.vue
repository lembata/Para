...
const GetButtonTooltip = (key) => {
	return {value: t(key), showDelay: 500}
}
			...
			<Column field="actions" :header="t('actions')" key="actions">
				<template #body="slotProps">
					<RouterLink :to="'/accounts/' + slotProps.data.id">
						<Button v-tooltip.top="GetButtonTooltip('details')" icon="pi pi-eye" rounded/>
					</RouterLink>
					<RouterLink :to="'/accounts/edit/' + slotProps.data.id">
						<Button v-tooltip.top="GetButtonTooltip('edit')" icon="pi pi-pencil" rounded/>
					</RouterLink>
					<Button severity="danger"
					        icon="pi pi-trash"
					        v-tooltip.top="GetButtonTooltip('delete')"
